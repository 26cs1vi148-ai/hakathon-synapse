...
const sosSchema = z.object({
  ...
  hostel: z.string().trim().min(1).max(120),
...
const statusSchema = z.object({
  status: z.enum(['ACTIVE', 'ATTENDING', 'SAFE', 'RESPONDED', 'RESOLVED']),
  note: z.string().trim().max(2000).default(''),
  wardenEmail: z.string().trim().email().max(200).optional()
});
  ...
  try {
    ...
    if (existing) {
      return res.status(409).json({
        message: 'This student already has an active SOS.',
        sosId: existing.id
      });
    }
    ...
    publish('sos.created', alert);
    publish('sos.location', alert);
...
export async function updateStatus(req, res, next) {
  try {
    const { status, note, wardenEmail } = statusSchema.parse(req.body);
    ...
    await store.addWardenAction(alert.id, {
      type: 'STATUS_CHANGE',
      status,
      wardenEmail: wardenEmail || 'Unknown warden',
      timestamp: now,
      note: note || ''
    });

    if (note) {
      await store.addWardenNote(alert.id, {
        text: note,
        wardenEmail: wardenEmail || 'Unknown warden',
        timestamp: now
      });
    ...
    const finalAlert = await store.getAlert(alert.id);

    publish('sos.status', finalAlert);
...
export async function createDemo(req, res, next) {
  try {
    ...
    if (existing) {
      return res.status(409).json({
        message: 'Demo SOS already active.',
        sosId: existing.id
      });
    }
...
export async function simulateLocation(req, res, next) {
  try {
    ...
    const lat = alert.latitude + 0.00005;
    const lng = alert.longitude + 0.000035;
    const now = new Date().toISOString();
